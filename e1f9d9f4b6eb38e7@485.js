...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      "census-regions.csv",
      new URL(
        "./files/5b9c7bae66b39d0cf633607b7e984064ba0f5300b74a157724fc5c5393268927d667bb69319dc7122f660786411b6599167101662fcd0ddf90895814707e6608",
        import.meta.url
      ),
    ],
    [
      "population.tsv",
      new URL(
        "./files/2917384d5b3890d62715d46d1ce13c242b26c4b78ac2300859112842c1e251af820a09d160a4b7cfa323dc82b863e738bdf58e08eb4cffe640c7b4bbc3e25a85",
        import.meta.url
      ),
    ],
  ]);
  ...
  //   });
  main
    .variable(observer("viewof index"))
    .define(
      "viewof index",
      ["Scrubber", "d3", "data", "duration"],
      function (Scrubber, d3, data, duration) {
        return Scrubber(d3.range(data.keys.length), {
          delay: duration,
          loop: false,
  ...
  main
    ...
    .define("index", ["Generators", "viewof index"], (G, _) => G.input(_));
  main
  ...
  main
    ...
    .define("update", ["chart", "index"], function (chart, index) {
      return chart.update(index);
    });
  ...
  main
    .variable(observer("sums"))
    .define("sums", ["data", "d3"], function (data, d3) {
      return data.keys.map(
        (d, i) => d3.hierarchy(data.group).sum((d) => d.values[i]).value
      );
    });
  main
    .variable(observer("max"))
    .define("max", ["d3", "sums"], function (d3, sums) {
      return d3.max(sums);
    });
  main
  ...
  main
    ...
    .define(
      ...
      async function (d3, FileAttachment) {
        ...
        const divisionByState = new Map(
          regions.map((d) => [d.State, d.Division])
        );
        return {
          keys,
          group: d3.group(
